...
public class QueueController {
    ...
    @PostMapping("/redis/{topic}")
    public CommonResult<Boolean> sendRedisMess(@PathVariable("topic") String topic, @RequestParam("message") String message) {
    ...
    public CommonResult<Boolean> sendRabbitTTLMess() {
        rabbitProducer.sendTTLMess("rabbit延时(TTL)-消息内容");
        return CommonResult.success(true);
    }
    ...
    public CommonResult<Boolean> sendX_DelayMess() {
        rabbitProducer.sendX_DelayMess("rabbit延时(x-delayed)-消息内容");
        return CommonResult.success(true);
    }
