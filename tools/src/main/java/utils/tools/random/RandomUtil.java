...
public class RandomUtil extends RandomStringUtils {
	...
	static{
		...
		try {
			URI resource = URLUtil.url("E:/jimmy/tools_project/tools/src/main/resources").toURI();
			...
			ADDRESS_LIST = StringUtils.split(IoUtil.read(addressURI.toURL().openStream(), charset), ",");
			CITY_LIST = JSONObject.parseObject(IoUtil.read(citylistURI.toURL().openStream(), charset));
			AREANO_MAP = JSONObject.parseObject(IoUtil.read(idcodeURI.toURL().openStream(),charset));
			JOBS = StringUtils.split(IoUtil.read(jobURI.toURL().openStream(), charset),',');
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static String username(){
		...
		int secondNameLength = (int) randomNumber(1) + 1;
